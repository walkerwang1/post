...
import com.paper.alg.ch1.WKCRO.Molecule;
import com.paper.alg.ch1.WKCRO.VM;

public class AllInCloud {
	...
	public static void main(String[] args) {
		AllInCloud allInMobile = new AllInCloud();
		allInMobile.run();
	}
	...
	public void mobileResult() {
		for(int i = 1; i <= USER_NUM; i++) {
			...
			users[i].component[0].exeloc = 0;
			users[i].component[COMPENENT_NUM-1].exeloc = 0;
			for(int j = 1; j < COMPENENT_NUM-1; j++) {
				users[i].component[j].exeloc = 1;
			}
	...
	public double compMakeSpan(int i) {
		...
		for(int j = 0 ; j < COMPENENT_NUM; j++) {
			if (j == 0) { 	//第一个组件
				...
				int prepos = users[i].component[j-1].exeloc;
				int curpos = users[i].component[j].exeloc;
				if (prepos == 0 && curpos == 1) {
